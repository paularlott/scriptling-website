...
- [scriptling.ai](../../ai/): re-exports the class as `ai.ToolRegistry`, plus completion helpers and `execute_tool_calls`.
- [scriptling.ai.agent](../agent/): the `Agent` class consumes a `Registry` via its `tools=` argument.
- [scriptling.mcp.tool](../../mcp/tool/): building tools for the MCP protocol instead of direct LLM APIs.
