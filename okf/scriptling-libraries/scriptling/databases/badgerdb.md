...
resource: https://scriptling.dev/reference/libraries/scriptling/databases/badgerdb/
sources:
    - resource: https://scriptling.dev/reference/libraries/scriptling/databases/badgerdb/
status: stable
tags:
...
`scriptling.badgerdb` embeds BadgerDB as a local key/value store, no server needed. The string-KV core is mirrored exactly by [scriptling.valkey](https://scriptling.dev/okf/scriptling-libraries/scriptling/databases/valkey.md), so those scripts move between local storage and a shared cache by changing the open line and nothing else: strings, hashes, batch operations, counters and TTLs. Valkey's sets, queues and database selection have no counterpart here; badgerdb implements what the store does natively.
...
Identical surface to the [valkey Client](https://scriptling.dev/okf/scriptling-libraries/scriptling/databases/valkey.md#client):
...
- [Valkey](https://scriptling.dev/okf/scriptling-libraries/scriptling/databases/valkey.md): the same string-KV core against a server, plus sets and queues
- [Database Libraries](https://scriptling.dev/okf/scriptling-libraries/scriptling/databases/badgerdb.md): all four backends, two API shapes
