...
resource: https://scriptling.dev/reference/libraries/utilities/find/
sources:
    - resource: https://scriptling.dev/reference/libraries/utilities/find/
status: stable
tags:
...
When searching recursively, entries are stat'd and filtered concurrently using the same bounded worker-pool model as [`scriptling.grep`](https://scriptling.dev/okf/scriptling-libraries/utilities/grep.md), so large trees are scanned in parallel.
...
- [scriptling.grep](https://scriptling.dev/okf/scriptling-libraries/utilities/grep.md): File content search, using the same parallel worker model
