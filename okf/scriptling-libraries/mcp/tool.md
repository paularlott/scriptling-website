...
resource: https://scriptling.dev/reference/libraries/mcp/tool/
sources:
    - resource: https://scriptling.dev/reference/libraries/mcp/tool/
status: stable
tags:
...
For the MCP client library (connecting to MCP servers), see [scriptling.mcp](https://scriptling.dev/okf/scriptling-libraries/mcp/client.md).
...
When the MCP server is served over HTTP, these give tool scripts a look at the HTTP request the call arrived on — the same one the server's [middleware](https://scriptling.dev/okf/scriptling-libraries/runtime/http.md#middlewarehandler) saw. Over the stdio transport there is no HTTP request.

### `mcp.tool.get_request()`

Returns the HTTP request this tool call is being served for: a [Request object](https://scriptling.dev/okf/scriptling-libraries/runtime/http.md#request-object) with `method`, `path`, `headers`, `query`, `remote_addr` and `context`. Returns `None` over stdio or anywhere else outside a served request.
