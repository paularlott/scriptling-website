...
resource: https://scriptling.dev/docs/security/
sources:
    - resource: https://scriptling.dev/docs/security/
status: stable
tags:
...
- **CLI**: `--network-policy=policy.toml` — the full policy file reference is in the [network policy guide](https://scriptling.dev/okf/scriptling-docs/cli/network-policy.md). Combine it with `--disable-lib subprocess` in any mode so code cannot bypass the policy by shelling out.
